...
static inline struct dl_list *select_bucket(struct hashmap *hmap,
                                               uint32_t hash)
{
    return &hmap->buckets[hash & (hmap->nb_buckets - 1)];
}

static inline struct hmap_elem* lookup_elem_raw(struct dl_list *head,
...
ubpf_hashmap_lookup(const struct ubpf_map *map, const void *key)
{
    ...
    struct dl_list *head = select_bucket(hmap, hash);
...
ubpf_hashmap_update(struct ubpf_map *map, const void *key, void *value)
{
    ...
    if (!old_elem && (hmap->count >= map->max_entries)) {
        return -4;
    }
    ...
    dl_list_insert(head, &new_elem->hash_node);
    if (old_elem) {
        dl_list_remove(&old_elem->hash_node);
        free(old_elem);
    } else {
        hmap->count++;
    }
